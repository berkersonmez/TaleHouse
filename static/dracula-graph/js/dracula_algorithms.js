...
function selectionSort(arr) {
    ...
    for (var i in arr) {
        if (arr[i] < minimum) {
            minimum = arr[i];
            index = i;
            /* remember the minimum index for later removal */
        }
    }
    /* remove the minimum */
    arr.splice(index, 1);
...
function mergeSort(arr) {
    ...
    var part1 = arr.slice(0, median);
    /* for some reason it doesn't work if inserted directly in the return statement (tried so with firefox) */
